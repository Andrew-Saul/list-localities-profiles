...
+ A total population of **`r total_population[[1]]`** people, where **`r gender_breakdown[gender_breakdown$sex == "M" & gender_breakdown$locality == locality_list[1],]$perc`** were male, and **`r over65[[1]]`%** were aged over 65.
...
+ A total population of **`r total_population[[2]]`** people, where **`r gender_breakdown[gender_breakdown$sex == "M" & gender_breakdown$locality == locality_list[2],]$perc`** were male, and **`r over65[[2]]`%** were aged over 65.
...
In `r pop_max_year`, the total population of `r locality_list[1]` locality was `r total_population[[1]]`. The graph below shows the population distribution of the locality. Overall, **`r gender_breakdown[gender_breakdown$sex == "M" & gender_breakdown$locality == locality_list[2],]$perc`** of the population are male, and **`r gender_breakdown[gender_breakdown$sex == "F" & gender_breakdown$locality == locality_list[2],]$perc`** are female.

#### Figure `r x`: Population by age and sex.

\newline

#### Figure `r x`: Population by age and sex in `r locality_list[1]` and `r locality_list[2]`.
```{r pyramid-clyde, echo = FALSE, fig.width = 7, fig.height = 3.5, out.width="50%" , warning = FALSE, results='hold'}
pop_pyramid_plot[[1]] 
pop_pyramid_plot[[2]] 
...
Figure `r x` shows the historical population of `r locality_list[1]`, along with the NRS population projections. While the population in `r locality_list[2]` locality appeared stable for the period 2011-2022, there appeared a decreasing population trend in the `r locality_list[1]` locality over the same period. Noted in the `r locality_list[1]` locality in 2021 was a large population drop that recovered in 2022, possibly associated with the COVID pandemeic.  `r pop_graph_text` `r pop_proj_text`. *Please see the footnotes for more information on how the population projections were calculated^1^.* 
...
Figure `r x` shows how the population structure has changed between `r pop_min_year` and `r pop_max_year` for both `r locality_list[1]` and `r locality_list[2]`.

#### Table `r y`: Population time trend and projection values.

```{r clydebank-pop-table, echo=FALSE, out.width="80%"}
actual_pop
est_pop
...
```{r histpop, echo = FALSE, fig.width = 7, fig.height = 3.5, out.width="50%" , warning = FALSE, fig.show='hold'}
hist_pop_change_plots[[1]]
hist_pop_change_plots[[2]]
...
### Deprivation
The following section explores the deprivation structure of `r locality_list[1]` through the Scottish Index of Multiple Deprivation (SIMD). The SIMD ranks all datazones in Scotland by a number of factors; Access, Crime, Education, Employment, Health, Housing and Income. Based on these ranks, each datazone is then given an overall deprivation rank, which is used to split datazones into Deprivation Quintiles (Quintile 1 being the most deprived, and Quintile 5 the least). The most recent SIMD ranking was carried out in 2020. This section mainly focuses on the SIMD 2020 classifications, however the 2016 classifications are used to assess how deprivation has changed in `r locality_list[1]` when compared to the rest of Scotland.

Of the `r pop_max_year` population in `r locality_list[1]`, **`r perc_bottom_quintile[[1]]`%** live in the most deprived Quintile (SIMD 1), and **`r perc_top_quintile[[1]]`%** live in the least deprived Quintile (SIMD 5). For `r locality_list[2]`, **`r perc_bottom_quintile[[2]]`%** live in the most deprived Quintile (SIMD 1), and **`r perc_top_quintile[[2]]`%** live in the least deprived Quintile (SIMD 5)


#### Figure `r x`: Map of Data Zones, coloured by SIMD  quintiles.

```{r simdmap, echo = FALSE, fig.width = 7, fi.height = 7, warning = FALSE,  out.width="50%", results='hold'}
simd_map[[1]]
simd_map[[2]]
...
<!-- #### Figure `r x`: Map of Data Zones within `r locality_list[1]` coloured by SIMD  quintiles. -->
...
<!-- #### Figure `r x`: Map of Data Zones within `r locality_list[2]` coloured by SIMD  quintiles. -->
...
The following tables details the percentage of the 2016 population living in the 2016 SIMD Quintiles, the percentage of the `r pop_max_year` population living in the 2020 SIMD Quintiles, and their difference for comparison. Figure `r x` then breaks down SIMD by domain in `r locality_list[1]`.

#### Table `r y`: Percentage population living in `r locality_list[1]` in the 2016 and 2020 SIMD Datazone Quintiles in 2016 and `r pop_max_year` respectively.
\newline
...
```
\newline
#### Table `r y`: Percentage population living in `r locality_list[2]` in the 2016 and 2020 SIMD Datazone Quintiles in 2016 and `r pop_max_year` respectively.
\newline
...
```{r simdomplots, echo = FALSE, fig.width = 10, fig.height = 4, warning = FALSE}
sim_dom_plots <- 
...
sim_dom_plots <- 
  wrap_elements(panel = sim_dom_plots) +
  labs(tag = "Proportion of Population") +
  theme(
...
Figures `r x` and `r x+1` presents a comparison between the 2016 SIMD ranking applied to 2016 population estimates, and the more recent 2020 SIMD ranking applied to `r pop_max_year` population estimates for `r locality_list[1]` and `r locality_list[2]` localities. The percentages of the population living within each SIMD quintile by domain were calculated using the 2016 and 2020 SIMD datazone classifications respectively. The differences in these percentages for `r locality_list[1]` and `r locality_list[2]` are plotted in Figures `r x` and `r x+1`, respectively. Negative values on the y-axis indicate a decrease in percent of the population living within a quintile, while positive values indicate an increase in percent of the population living within a quintile. **Please note that quintiles have been weighted by the Scottish population so, any local changes in SIMD quintile do not necessarily indicate a difference in deprivation, but rather a difference in deprivation in comparison to the rest of Scotland.**

\newline

#### Figure `r x`: Percentage `r locality_list[1]` population in 2016 and `r pop_max_year` living in the 2016 and the 2020 SIMD quintiles by domain.

\newline

```{r simddiffplot1, echo = FALSE, fig.width = 7.5, fig.height = 7, warning = FALSE}
simd_diff_plot[[1]]
x <- x + 1
```
...
#### Figure `r x`: Percentage `r locality_list[2]` population in 2016 and `r pop_max_year` living in the 2016 and the 2020 SIMD quintiles by domain.

\newline

```{r simddiffplot2, echo = FALSE, fig.width = 7.5, fig.height = 7, warning = FALSE}
simd_diff_plot[[2]]
x <- x + 1
```
...
#### Table `r y`: Number of each type of service in `r locality_list[1]` and `r locality_list[2]` Localities^2^.
\newline

```{r echo = FALSE}
knitr::kable(services_tibble, col.names = c(paste0("**Service ",colnames(services_tibble)[1], "**"),
                                            paste0("**",colnames(services_tibble)[2], "**"),
                                            paste0("**",colnames(services_tibble)[3], "**"), 
                                            paste0("**",colnames(services_tibble)[4], "**")
                                            ),
             format = "markdown")
...
In `r latest_year_access_dep`, `r format_number_for_text(access_dep_latest[[1]])`% of the population of `r locality_list[1]` lived in the 15% most 'access deprived' areas ^3^. In the same year, `r format_number_for_text(access_dep_latest[[2]])`% of the population of `r locality_list[2]` lived in the 15% most 'access deprived' areas ^3^.
...
For the most recent time period available, `r locality_list[1]` Locality had:
 ...
 - **`r text_values[[1]]$n_houses`** dwellings, of which: **`r text_values[[1]]$perc_occupied`%** were occupied and **`r text_values[[1]]$perc_second_homes`%** were second homes.
 - **`r text_values[[1]]$perc_single_discount`%** of dwellers received a single-occupant council tax discount, and **`r text_values[[1]]$perc_exempt`%** were exempt from council tax entirely.
 - **`r text_values[[1]]$perc_houses_AC`%** of houses were within council tax bands A to C, and **`r text_values[[1]]$perc_houses_FH`%** were in bands F to H.

\newline

For the most recent time period available, `r locality_list[2]` Locality had:

 - **`r text_values[[2]]$n_houses`** dwellings, of which: **`r text_values[[2]]$perc_occupied`%** were occupied and **`r text_values[[2]]$perc_second_homes`%** were second homes.
 - **`r text_values[[2]]$perc_single_discount`%** of dwellers received a single-occupant council tax discount, and **`r text_values[[2]]$perc_exempt`%** were exempt from council tax entirely.
 - **`r text_values[[2]]$perc_houses_AC`%** of houses were within council tax bands A to C, and **`r text_values[[2]]$perc_houses_FH`%** were in bands F to H.

\newline


The graph below shows the number of dwellings in `r locality_list[1]` and `r locality_list[2]` from `r min(house_dat$year)` to `r max(house_dat$year)`.

#### Figure `r x`: Number of dwellings time trend.
\newline

```{r houseinsetplot, echo = FALSE, fig.width = 8.5, fig.height = 4, warning = FALSE}
houses_ts + inset_element(houses_ts_inset, left = 0.05, top = 0.7, right=1, bottom = 0.05)

x <- x + 1
```

##### `r locality_list[1]`
Of the total number of dwellings in `r max(house_dat$year)`, `r text_values[[1]]$perc_single_discount`% (`r text_values[[1]]$n_single_discount` households) were occupied by an individual receiving a single-occupant council tax discount. Furthermore, `r text_values[[1]]$perc_exempt`% (`r text_values[[1]]$n_exempt` households) were occupied and exempt from council tax.

There were `r text_values[[1]]$n_second_homes` dwellings classed as a second home in `r max(house_dat$year)`, these dwellings made up `r text_values[[1]]$perc_second_homes`% of total households.

##### ##### `r locality_list[2]`
Of the total number of dwellings in `r max(house_dat$year)`, `r text_values[[2]]$perc_single_discount`% (`r text_values[[2]]$n_single_discount` households) were occupied by an individual receiving a single-occupant council tax discount. Furthermore, `r text_values[[2]]$perc_exempt`% (`r text_values[[2]]$n_exempt` households) were occupied and exempt from council tax.

There were `r text_values[[2]]$n_second_homes` dwellings classed as a second home in `r max(house_dat$year)`, these dwellings made up `r text_values[[2]]$perc_second_homes`% of total households.
...
The proportion of households within each council tax band are displayed in the charts `r x` and `r x+1` below, figures are shown in Table `r y`.

#### Figure `r x`: Breakdown of households by council tax band for `r locality_list[1]` in `r max(housing_sheets)`.
\newline

```{r ctplot1, echo = FALSE, fig.width = 7, fig.height = 2, warning = FALSE}
ctb_plot[[1]]

x <- x + 1
```

#### Figure `r x`: Breakdown of households by council tax band for `r locality_list[2]` in `r max(housing_sheets)`.
\newline

```{r ctplot2, echo = FALSE, fig.width = 7, fig.height = 2, warning = FALSE}
ctb_plot[[2]]

x <- x + 1
```

#### Table `r y`: Percentage of households by council tax band for `r locality_list[2]` and `r locality_list[2]` in `r max(housing_sheets)`.
\newline

```{r ctbtable, echo = FALSE}
